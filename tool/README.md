...
这是一个由 `Ask DeepSeek` 技能调用的 DeepSeek 网页端适配器，借鉴 Oracle 的两段式流程。自动发送默认由 Codex 内置浏览器完成：

1. 本地将问题和指定文件渲染为带行号的 Markdown 包。
2. 仅在你显式允许时，使用 Codex 内置浏览器连接 `chat.deepseek.com`。
...
技能层的自动发送使用 Codex 内置浏览器；本目录中的 `ask` / `recover` 仍是旧版 EGO 适配器接口，不应被用来代替内置浏览器。只有用户明确要求批量/脚本化，或确认内置浏览器不可用并同意备用通道时，才使用该 EGO 接口。`launch` / `probe` 是独立 Chrome + CDP 的低层诊断命令，仅用于诊断。
...
在此目录执行（或从技能目录调用）本地渲染。下面是不带参数时的默认专家模式；渲染不会联网：
...
发送请回到 Codex 内置浏览器：打开或复用 `https://chat.deepseek.com/`，读取可访问性状态，确认唯一的“专家模式”或“快速模式” radio，使用语义控件填入问题并发送，再读取稳定回答。独立 Node CLI 无法取得内置浏览器句柄，因此不要把 `ask` 子命令当作内置浏览器发送通道。
...
- 支持用户明确选择的专家模式或快速模式；自动发送由 Codex 内置浏览器完成，不支持其他模式的自动发送。
