...
- 默认使用 EGO Lite 中继承登录态的独立任务空间。EGO Lite 不可用时，说明原因后才改用其他已授权浏览器或手动粘贴。
...
2. **打开并核验页面。** 按 `ego-browser` 技能打开或复用 `https://chat.deepseek.com/`。确认已登录、页面只有一个目标输入框；未登录时把该任务空间交给用户正常登录，不索取凭证。
...
用户接管 EGO Lite 后是硬停止；只有用户明确说继续，才能按 `ego-browser` 的接管流程恢复。成功后关闭本任务创建且无需保留的任务空间。
...
适配器的 `ask` 使用 EGO Lite 和统一模型页面契约，并保留页面已有的独立开关状态。用户要求精确开关状态时，使用上面的浏览器流程设置并复核，不接受旧版模式参数。
